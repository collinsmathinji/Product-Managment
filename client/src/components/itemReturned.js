...
const ItemReturned = () => {
  const [formData, setFormData] = useState({
    employeeId: '',
    items: [{ itemId: '', quantityReturned: 0 }],
  });
  ...
  const handleAddItem = () => {
    setFormData({ ...formData, items: [...formData.items, { itemId: '', quantityReturned: 0 }] });
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/transactions/return', formData);
        ...
        <form onSubmit={handleSubmit}>
          ...
          {formData.items.map((item, index) => (
            <div key={index} className="mb-4">
              ...
              />
              <label className="block mb-2 text-sm" htmlFor={`quantityReturned-${index}`}>
                Quantity Returned
              </label>
              <input
                type="number"
                name="quantityReturned"
                id={`quantityReturned-${index}`}
                placeholder="e.g. 10"
                className="rounded-md p-2 w-full"
